...
const float HEAD_MIN             = -90.0;
const float HEAD_MAX             =  90.0;
const float SHOULDER_PITCH_MIN   = -10.0;
...
const float ELBOW_MIN            =  -35.0;
const float ELBOW_MAX            = 35.0;
const float KNEE_MIN             =  0.0;
const float KNEE_MAX             = 75.0;
const float LIGHT_DEG_MIN        = -90.0;
const float LIGHT_DEG_MAX        = 270.0;
const float LIGHT_RAD            = 40;

float LIGHT_DEG = -90;

GLfloat matte_shininess = 0.6 * 128;

GLfloat metallic_shininess = 0.7 * 128;

GLfloat metallic_diffuse[] = { 0.2, 0.2, 0.2, 0.2f },
 metallic_ambient[] = { 0.9, 0.9, 0.9, 1.0f },
 metallic_specular[] = { 0.7, 0.7, 0.7, 1.0f };

GLfloat matte_diffuse[] = { 0.7, 0.7, 0.7, 1.0 },
 matte_ambient[] = { 0.7, 0.7, 0.7, 1.0 },
 matte_specular[] = { 0.2, 0.2, 0.2, 0.7 };
...
void drawWeightedPrism(float xtop, float xbot, float ytop, float ybot, float ztop, float zbot, bool checkOutlined);
...
void initGlui()
{
    ...
    ///////////////////////////////////////////////////////////
        ...
        // Create controls to specify left leg
    glui_panel = glui_joints->add_panel("Lighting");

    glui_spinner = glui_joints->add_spinner_to_panel(glui_panel, "Light DEG:", GLUI_SPINNER_FLOAT, &LIGHT_DEG);
    glui_spinner->set_float_limits(LIGHT_DEG_MIN, LIGHT_DEG_MAX, GLUI_LIMIT_CLAMP);
    glui_spinner->set_speed(SPINNER_SPEED);
...
void setLightSource(void) {
    if (renderStyle == METALLIC || renderStyle == MATTE){
        ...
        glEnable(GL_LIGHTING);
        glEnable(GL_COLOR_MATERIAL);
        float pos[] = { LIGHT_RAD * cosf(LIGHT_DEG*PI / 180),
                LIGHT_RAD * sinf(LIGHT_DEG*PI / 180),
                100.0, 1.0 };
...
part head = {
    0.9,1.3,0.7,0.7,0.9,1.3,
};
part upperBeak = {
    0.3,0.43,0.1,0.1,0.1,0.1,
};
part lowerBeak = {
    0.4,0.3,0.1,0.1,0.1,0.1,
};
part arm = {
    0.8,0.5,0.0,2.0,0.1,0.1,
};
part hand = {
    0.5,0.5,0.0,0.5,0.1,0.1,
};
part leg = {
    0.1,0.1,0.0,1.5,0.1,0.1,
};
part feet = {
    0.0,0.4,0.0,0.5,0.05,0.05,
};

void drawPenguin() {
    if (renderStyle != OUTLINED) glColor3f(0.5,0.5,0.5);
    drawWeightedPrism(body.xtop, body.xbot, body.ytop, body.ybot, body.ztop, body.zbot, true);
    glPushMatrix();
        if (renderStyle != OUTLINED) glColor3f(0.2,0.2,0.2);
        glRotatef(joint_ui_data->getDOF(Keyframe::HEAD), 0, 1, 0);
        glTranslatef(0.0, body.ytop + head.ybot, 0.0);
        drawWeightedPrism(head.xtop, head.xbot, head.ytop, head.ybot, head.ztop, head.zbot, true);
        glPushMatrix();
            if (renderStyle != OUTLINED) glColor3f(0.3,0.3,0.3);
            glTranslatef(-((head.xtop+head.xbot)/2)-((upperBeak.xtop+0.35)/2), 0.0, 0.0);
            drawWeightedPrism(upperBeak.xtop, upperBeak.xbot, upperBeak.ytop, upperBeak.ybot, upperBeak.ztop, upperBeak.zbot, true);
            // drawWeightedPrism(0.3,0.43,0.1,0.1,0.1,0.1,true);
        glPopMatrix();

        glPushMatrix();
            if (renderStyle != OUTLINED) glColor3f(0.3,0.3,0.3);
            glTranslatef(-((head.xtop+head.xbot)/2)-((lowerBeak.xbot+0.35)/2), -(0.1+0.1)-joint_ui_data->getDOF(Keyframe::BEAK), 0.0);
            drawWeightedPrism(lowerBeak.xtop, lowerBeak.xbot, lowerBeak.ytop, lowerBeak.ybot, lowerBeak.ztop, lowerBeak.zbot, true);
        glPopMatrix();

    glPopMatrix();

    glPushMatrix();
        if (renderStyle != OUTLINED) glColor3f(0.4,0.4,0.4);
        glTranslatef(0.0, 1.5, 1.2);
        glRotatef(-26.57,1,0,0);
        glRotatef(-10,0,0,1);
        ...
        // drawWeightedPrism(lowerBeak.xtop, lowerBeak.xbot, lowerBeak.ytop, lowerBeak.ybot, lowerBeak.ztop, lowerBeak.zbot, true);
        drawWeightedPrism(0.8,0.5,0.0,2.0,0.1,0.1,true);

        glPushMatrix();
            if (renderStyle != OUTLINED) glColor3f(0.6,0.6,0.6);
            glTranslatef(0.0, -2.0, 0.0);
            glRotatef( -joint_ui_data->getDOF(Keyframe::L_ELBOW)-10.0, 0, 0, 1);
            // drawWeightedPrism(lowerBeak.xtop, lowerBeak.xbot, lowerBeak.ytop, lowerBeak.ybot, lowerBeak.ztop, lowerBeak.zbot, true);
            drawWeightedPrism(0.5,0.5,0.0,0.5,0.1,0.1,true);
        glPopMatrix();
    glPopMatrix();
    ...
    glPushMatrix();
        ...
        glPushMatrix();
            if (renderStyle != OUTLINED) glColor3f(0.6,0.6,0.6);
            glTranslatef(0.0, -2.0, 0.0);
            glRotatef( -joint_ui_data->getDOF(Keyframe::R_ELBOW)-10.0, 0, 0, 1);
            drawWeightedPrism(0.5,0.5,0.0,0.5,0.1,0.1,true);
        glPopMatrix();
    glPopMatrix();

    glPushMatrix();
        if (renderStyle != OUTLINED) glColor3f(0.7,0.7,0.7);
        ...
        drawWeightedPrism(0.1,0.1,0.0,1.5,0.1,0.1,true);

        glPushMatrix();
            if (renderStyle != OUTLINED) glColor3f(0.8,0.8,0.8);
            glTranslatef(0.0, -1.5, 0.0);
            glScalef(2.0, 1.0, 1.0);
            glRotatef(90.0,1,0,0);
            glRotatef(-90.0,0,0,1);
            glRotatef(joint_ui_data->getDOF(Keyframe::R_KNEE),1,0,0);
            drawWeightedPrism(0.0,0.4,0.0,0.5,0.05,0.05,true);
        glPopMatrix();
    glPopMatrix();
